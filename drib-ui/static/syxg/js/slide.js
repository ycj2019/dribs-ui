!function (t) {
    ...
    A.prototype = {
        init: function (element, data) {
            this.element = element, this.data = $.extend({}, {
                direction: "right",
                ratio: 16 / 9,
                ime: 3000,
                base: !0,
                ...
                over: !0,
                stop: !0
            }, data), this.box_left = parseInt(element.css('width')), this.list_left = -this.box_left, this.list_child = element.find('ul').children().length;
            var first_child = this.element.find('li')[0];
            var last_child = this.element.find('li')[this.list_child - 1];
            this.element.find('ul').append(first_child.cloneNode(true)), this.element.find('ul').prepend(last_child.cloneNode(true)), this.list_child = this.element.find('ul').children().length, this.assembly()
        }, assembly: function () {
            this.adaption();
            this.btn();
            ...
            function auto() {
                if (direction === "right") {
                    ...
                    element.find('.prev').trigger("click");
                }
                else {
                    element.find('.next').trigger("click");
                }
